...
void menu() 
{
    ...
    do 
    {
        ...
        printf(" MENU SESION 3: COLA DINAMICA CON SIMULACION\n\n");
        printf(" 1.- Agregar trabajo (Enqueue)\n");
        printf(" 2.- Ver siguiente trabajo (Peek)\n");
        printf(" 3.- Procesar e Imprimir (Sesion 3)\n");
        printf(" 4.- Mostrar cola completa\n");
        printf(" 0.- Salir\n");

        opcion = validacion_num(0, 4, " Elija la accion: ");
        ...
        switch (opcion) 
        {
            case 1:
                ...
                nuevo.id = id_counter++;
                printf("Usuario: ");
                my_gets(nuevo.usuario, MAX_USER);
                printf("Documento: ");
                my_gets(nuevo.documento, MAX_DOC);
                
                nuevo.paginas_total = validacion_num(1, 100, "Paginas (1-100): ");
                nuevo.paginas_restantes = nuevo.paginas_total;
                nuevo.copias = 1;
                nuevo.prioridad = NORMAL;
                nuevo.estado = EN_COLA;
                nuevo.ms_por_pagina = 400;

                if (qd_enqueue(&cola, nuevo)) 
                {
                    printf("\nTrabajo ID %d encolado correctamente.\n", nuevo.id);
                }
            ...
            case 2:
                ...
                {
                    printf("SIGUIENTE: [%d] %s | Pags: %d\n", job_aux.id, job_aux.documento, job_aux.paginas_total);
                }
                else 
                {
...
void qd_print(const QueueDynamic_t* q) 
{
    ...
    while (actual) 
    {
        printf("[ID:%d | %s | %d pags]\n", actual->data.id, actual->data.usuario, actual->data.paginas_total);
        actual = actual->next;
    }
}
...
void qd_destroy(QueueDynamic_t* q) 
{ 
    PrintJob_t temp; while (qd_dequeue(q, &temp)); 
}
